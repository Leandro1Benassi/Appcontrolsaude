...
function Receber() {
    ...
    async function fetchData() {
        try {
            ...
            const response = await api.get(`receber/listar.php?data=${date1}&data1=${dates2}`);
    ...
    async function excluir(nome:string, id:string) {
        ...
        Alert.alert('Sair', `Você tem certeza que deseja excluir a Conta de valor : ` + nome, [
...

            {
                ...
                onPress: async () => {
                    try {
                        const res = await api.get(`receber/excluir.php?id=${id}&user=${user}`);
           ...
           >
                    <SwipeableRow 
                        onPressWhatsapp={async () => {
                            navigation.push('BaixarReceber', { id_reg: item.id, valor_conta: item.valor, multa: item.multa, juros: item.juros });
                        }}

                        onPressEdit={async () => {
                            navigation.push('NovaContaReceber', { id_reg: item.id });
                        }}
            ...
            {isLoading ?
                ...
                <View style={styles.containerFloat}>
                    <TouchableOpacity
                        ...
                        onPress={() => navigation.push("NovaContaReceber", { id_reg: '0' })}
                    >
                    ...
                    {(() => {
                        if (tel != '') {
                            return(
                                ...
                                onPress={async () => {
                                  await Linking.openURL(`http://api.whatsapp.com/send?1=pt_BR&phone=55${tel}&text=Ola, ${forn} Lembrete de vencimento no dia: ${venc} no valor de: R$ ${valor}`)
                              }}
                              >
