...
const NovoCliente: React.FC = () => {
  ...
  async function alterarQuant(id: string, quant: string, funcao: string) {
    try {
      const res = await api.get(`compras/definir-quantidade.php?id=${id}&quant=${quant}&funcao=${funcao}`);
  ...
  async function listarclientes() {
    try {
      ...
      const response = await api.get(`compras/listar-forn.php?pagina=${page}&limite=10`);
  ...
  async function listarprodutos() {
    try {
      ...
      const response = await api.get(`compras/listar-prod.php?pagina=${page}&limite=10`);
    ...
    <View style={{ flex: 1, marginTop: 20 }}>
      <View style={styles.Header}>
        <TouchableOpacity
          ...
          onPress={() => navigation.push("Compras")}
        >
      ...
      <RectButton
        style={styles.Button}
        onPress={() => {
          navigation.push("FecharCompra", { subTotal: totalVenda, cliente: idcliente} )
        }}
      >
