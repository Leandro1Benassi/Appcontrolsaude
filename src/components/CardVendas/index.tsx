    ...
    async function excluir(nome: string, id: string) {
        ...
        Alert.alert('Sair', `Você tem certeza que deseja cancelar a Venda : ` + nome, [
...

            {
                ...
                onPress: async () => {
                    try {
                        const res = await api.get(`vendas/excluir.php?id=${id}}&user=${user}`);
                ...
                <View>
                    <SwipeableRow
                        onPressWhatsapp={async () => {
                            await Linking.openURL(urlRaiz + `relatorios/venda_class.php?id=${data.id}`)
                        }}
                        ...
                        >
                            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
                                ...
                                <View style={{ width: '100%', marginTop: 3 }}>
                                    ...
                                    <Text> <Text><Ionicons name="square" size={20} color={data.cor} /> </Text> <Text style={{ color: '#013d10', fontSize:17 }}>R$ {data.subtotal} </Text> <Text style={{ color: '#000', fontSize:17 }}>- {data.cliente}</Text></Text>
